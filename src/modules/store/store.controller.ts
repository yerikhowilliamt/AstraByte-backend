...
export class StoreController {
  ...
  async create(
    ...
    this.loggerService.debug('STORE', 'controller', 'Creating new store initiated', {
      user_id: userId,
      name: request.name
    });
    
    this.loggerService.info('STORE', 'controller', 'Creating new store initiated', {
      user_id: userId,
    });
    ...
    try {
      ...
      this.loggerService.info('STORE', 'controller', 'Store created successfully', {
  ...
  async get(
    ...
    try {
      ...
      this.loggerService.info('STORE', 'controller', 'Store fetched successfully', {
        user_id: result.userId,
        store_id: result.id,
        response_status: 200,
      });
  ...
  async update(
    ...
    this.loggerService.debug('STORE', 'controller', 'Updating store initiated', {
      user_id: userId,
      store_id: storeId,
      name: request.name
    });
    ...
    try {
      ...
      this.loggerService.info('STORE', 'controller', 'Store updated successfully', {
        user_id: result.userId,
        store_id: result.id,
        response_status: 200,
      });
  ...
  async delete(
    ...
    try {
      ...
      this.loggerService.info('STORE', 'controller', 'Store deleted successfully', {
        user_id: userId,
        store_id: storeId,
        response_status: 200,
      });
  ...
  private checkAuthorization(userId: number, user: User): void {
    if (user.id !== userId) {
      this.loggerService.info('STORE', 'controller', 'Checking authorization initiated');

      this.loggerService.debug('STORE', 'controller', 'Chechking Authorization initiated', {
        user_id: user.id,
        email: user.email
      });
